...
return [
    ...
    'credentials' => [
        'key' => getenv('AWS_ACCESS_KEY_ID') ?: $AWS_ACCESS_KEY_ID,
        'secret' => getenv('AWS_SECRET_ACCESS_KEY') ?: $AWS_SECRET_ACCESS_KEY,
    ],

    // The AWS region where your bucket resides (e.g. "us‑east‑1").
    'region' => getenv('AWS_DEFAULT_REGION') ?: $AWS_DEFAULT_REGION,

    // The default API version to use. You normally don't need to change this.
    'version' => 'latest',

    // The name of the bucket that you want to manage.
    'bucket' => getenv('AWS_BUCKET_NAME') ?: $AWS_BUCKET_NAME,
    ...
    'endpoint' => getenv('AWS_ENDPOINT') ?: $AWS_ENDPOINT,
